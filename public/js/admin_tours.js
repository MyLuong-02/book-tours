document
  .getElementById("columnVisibilityButton")
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  function loadTourDifficulty() {
    ...
    difficultySelects.forEach(function (select) {
      ...
      var savedDifficulty = localStorage.getItem(
        "tour_" + tourId + "_difficulty"
      );
      if (savedDifficulty) {
    ...
    select.addEventListener("change", function () {
      ...
      if (newValue !== originalValue) {
        modalText.textContent =
          "Do you want to change the difficulty of this tour?";
  ...
  document.querySelectorAll("th[data-column]").forEach(function (header) {
    ...
    header.addEventListener("click", function () {
      ...
      rows.sort(function (a, b) {
        var aText = a.querySelector(
          "td:nth-child(" + (header.cellIndex + 1) + ")"
        ).innerText;
        var bText = b.querySelector(
          "td:nth-child(" + (header.cellIndex + 1) + ")"
        ).innerText;
  ...
  // Column Visibility
  document
    .querySelectorAll("#columnVisibilityDropdown a")
    .forEach(function (link) {
      link.addEventListener("click", function () {
        ...
        table.querySelectorAll("tbody tr").forEach(function (row) {
          row.querySelectorAll("td")[index].style.display = visible
            ? "none"
            : "";
        });
      });
